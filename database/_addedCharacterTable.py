...
def check_added_character_exsits(self, characterName):
    ...
    self.cursor.execute('''SELECT count(Name) FROM AddedCharacters WHERE Name=? ''', characterName)
...
def enter_added_new_character(self, character):
    self.cursor.execute('INSERT INTO AddedCharacters VALUES (?,?,?,?,?)', character)
    self.conn.commit()

def update_added_character_amount(self, character):
    self.cursor.execute('''UPDATE AddedCharacters SET Amount=?, Lewd=?, Wholesome=?, Duplicate=? WHERE Name=? ''', character)
    self.conn.commit()
...
def get_added_entry(self):
    characterInfo = self.cursor.execute('''SELECT * FROM AddedCharacters WHERE rowid = ? ''', self.currentId)
    self.currentId[0] += 1
...
def delete_added_character(self, character):
    self.cursor.execute('''DELETE FROM AddedCharacters WHERE Name=? ''', character)
    self.conn.commit()
...
def create_added_table(self):
    self.cursor.execute('''CREATE TABLE AddedCharacters(Name, Amount, Lewd, Wholesome, Duplicate)''')
